...
- None (corrected the mirrored autonomous-run-governance preset version)
Added sections:
- None
Removed sections:
- None
...
- None (no behavioral rule changed)
Follow-up TODOs:
- None
-->
...
- Projekt / Project: InventarWorkerService multi-project .NET 10 / C# 14 inventory solution.
- Laufzeit und Sprache / Runtime and language: ASP.NET Core worker/API, harvester, Terminal UI viewer, shared domain libraries, SQLite/MongoDB/PostgreSQL integration.
- Build und Tests / Build and tests: dotnet restore/build/test on InventarWorkerService.sln; MSTest unit and integration tests; Playwright setup for integration surfaces when required.
- Doku und A11Y / Docs and A11Y: DocFX output and learner-facing docs require text-oriented A11Y review; generated api/ and _site/ remain build artefacts.
- Statistik / Statistics: manual conservative baseline 80 lines/workday; repo-specific Thorsten-Solo baseline 100 lines/workday for this native .NET codebase unless updated in all agent files.
