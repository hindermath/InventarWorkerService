...
|---|---|
| Runtime / Language | .NET 10 / C# 14 multi-project inventory solution: worker/API, harvester, Terminal UI, shared libraries, SQLite/MongoDB/PostgreSQL |
| Build & Test Baseline | dotnet restore/build/test on InventarWorkerService.sln; MSTest unit/integration tests; Playwright setup when required |
| Docs / A11Y Baseline | DocFX output and learner-facing docs require text-oriented A11Y review; generated api/ and _site/ remain build artefacts |
| Statistics Baseline | Manual conservative 80; repo-specific Thorsten-Solo 100 lines/workday unless all agent files change it |
...
```text
Ersetzter Alt-Prompt: speckit-specify Nutze Lastenheft_Sandbox-gestuetzte-Secure-Development-Haertung.md als verbindliche Eingabedatei. Erstelle die Feature-Spezifikation fuer einen Sandbox-gestuetzten Secure-Development-Haertungslauf im Repository InventarWorkerService.

Ziel: Pruefe, wie InventarWorkerService sicher, nachvollziehbar und ausbildungsgeeignet in oder mit der absdd-image-sandbox bearbeitet werden kann. Starte keine Implementierung und fuehre keine technische Haertung aus.
...
- Projektkontext: .NET 10 / C# 14 multi-project inventory solution: worker/API, harvester, Terminal UI, shared libraries, SQLite/MongoDB/PostgreSQL.
- Build/Test-Baseline: dotnet restore/build/test on InventarWorkerService.sln; MSTest unit/integration tests; Playwright setup when required.
- Dokumentations- und A11Y-Basis: DocFX output and learner-facing docs require text-oriented A11Y review; generated api/ and _site/ remain build artefacts.
