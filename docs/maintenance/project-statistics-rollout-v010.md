...
Zweiter Kandidat des [Rollouts #302](https://github.com/hindermath/home-baseline/issues/302),
nach abgenommenem TinyPl0 PR #98. Ausgangscommit:
`106586833c7ae67635128d37ca2a3b66e38906b0`. Installation und Erstmessung sind
...
Second approved rollout target, following accepted TinyPl0 PR #98.
...
Die bisherige Profil-2-Statistik bleibt kanonisch (Europe/Berlin, 80/100).
Beide Verfahren schliessen den neuen Kontext aus. Unterschiedliche Aktivtage
sind moeglich und kein pauschaler Fehler. Kein weiterer Legacy-Ausschluss.

Use a separate UTC/52-week context without authored phase totals or reference
models. Preserve Profile 2 and its 80/100 references. Both exclude the new
context; timezone/methodology differences can change activity counts.
...
Bestehende .NET-Build-/Test-, Provider-, Security-, Coverage-/Supply-Chain- und
Docs-/A11Y-CI bleibt aktiv. Keine lokale Produkt-Build-Ausfuehrung und keine
Versionsaenderung: dies ist kein nummerierter Spec-Kit-Branch.
Vorhandener Release-PR bleibt unberuehrt.
...
Templates geprueft: Runtime, Produktarchitektur und 80/100-Basis bleiben
...
Leserechte und keine Secrets im Nachweis. OWASP ASVS 5.0 Level 2 bleibt fuer
die Produkt-API verbindlich; dieser Rollout aendert keine API-/Auth-Flaeche
und fuehrt keine neue ASVS-Abnahme durch. SBOM/Provenance des Presets bleiben
...
Product ASVS 5.0 Level 2 remains binding, without new API acceptance here.
