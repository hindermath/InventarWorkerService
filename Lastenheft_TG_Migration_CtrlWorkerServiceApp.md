...
```text
Ersetzter Alt-Prompt: speckit-specify Nutze Lastenheft_TG_Migration_CtrlWorkerServiceApp.md als verbindliche Eingabedatei. Erstelle die Feature-Spezifikation fuer einen Terminal.Gui-Migrations- und Kompatibilitaetslauf im Repository InventarWorkerService.
...
```text
$speckit-autonomous Execute one complete autonomous Spec Kit run using Lastenheft_TG_Migration_CtrlWorkerServiceApp.md as the binding intake. Delivery mode: LocalImplementation. Preserve all scope, ordering, security, accessibility, evidence, and acceptance boundaries. Do not push, create or merge a pull request, use bypass authority, expose secrets, or start a follow-up feature.
```
